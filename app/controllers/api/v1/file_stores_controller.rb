...
class Api::V1::FileStoresController < Api::ApplicationController
  ...
  def index
    fs = FileStore.by_hash(params[:hash]).page(params[:page])
  ...
  def create
    ...
    if file_store.save
      ...
      render json: { sha1_hash: file_store.sha1_hash }, status: :created #, location: @file_store
    else
    ...
    authenticate_or_request_with_http_basic do |user, pass|
      @res =
        Rails.cache.fetch(['Api::V1::FileStoresController#authenticate', user, pass], expires_in: 15.minutes) do
          uri  = URI.parse("#{App.abf_url}/api/v1/user.json")
